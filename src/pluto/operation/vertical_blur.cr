  ...
  def vertical_blur!(value : Int32) : Image
    ...
    channels.each do |channel|
      ...
      @width.times do |x|
        ...
        f_v = channel[c_i]
        l_v = channel[c_i + @width * (@height - 1)]
        c_v = (value + 1) * f_v

        (0..value - 1).each do |j|
          c_v += channel[c_i + j * @width]
        end
        (0..value).each do
          c_v += channel[r_i] - f_v
    ...
    @red = blurred_channels[0].to_a
    @green = blurred_channels[1].to_a
    @blue = blurred_channels[2].to_a
    @alpha = blurred_channels[3].to_a
